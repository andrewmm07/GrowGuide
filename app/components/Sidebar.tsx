...
export default function Sidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const navigation = [
    ...
    },
    {
      name: 'My Garden',
      href: '/my-garden',
      icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
    },
    {
      name: 'Tasks',
      href: '/tasks',
    ...
    },
    {
      name: 'Weather',
      href: '/weather',
      icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z" /></svg>
    },
    {
      name: 'Bed Buddies',
      href: '/bed-buddies',
    ...
    },
    {
      name: 'Common Issues',
      href: '/common-issues',
      icon: <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    },
    {
      name: 'Identify Issue',
      href: '/identify-issue',
  ...
  return (
    <>
      ...
      >
        ...
        {mounted && !loading && user && (
          <div className="border-t border-gray-200 p-4 flex-shrink-0 space-y-2">
            <Link href="/profile" className={`flex items-center gap-3 px-4 py-2 w-full rounded-md transition-colors ${pathname === '/profile' ? 'bg-green-50 text-green-700' : 'text-gray-700 hover:bg-gray-50'}`}>
            ...
            <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-2 w-full text-red-600 hover:bg-red-50 rounded-md transition-colors">
      ...
      <aside className={`hidden md:flex relative bg-white border-r transition-all duration-300 flex-col h-full ${isCollapsed ? 'w-16' : 'w-64'}`}>
        ...
        <nav className="py-4 flex-1 overflow-y-auto">
          {navigation.map((item) => (
            <Link key={item.name} href={item.href} className={navLinkClass(item.href)} title={isCollapsed ? item.name : undefined}>
              {item.icon}
        ...
        {mounted && !loading && user && (
          <div className="border-t border-gray-200 p-4 flex-shrink-0 space-y-2">
            <Link href="/profile" className={`flex items-center gap-3 px-4 py-2 w-full rounded-md transition-colors ${pathname === '/profile' ? 'bg-green-50 text-green-700' : 'text-gray-700 hover:bg-gray-50'} ${isCollapsed ? 'justify-center' : ''}`} title={isCollapsed ? 'Profile' : undefined}>
              <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              {!isCollapsed && <span>Profile</span>}
            </Link>
            <button onClick={handleLogout} className={`flex items-center gap-3 px-4 py-2 w-full text-red-600 hover:bg-red-50 rounded-md transition-colors ${isCollapsed ? 'justify-center' : ''}`} title={isCollapsed ? 'Log out' : undefined}>
              <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
