...
function YouTubeResource({ url, title }: { url: string; title: string }) {
  ...
  if (Capacitor.isNativePlatform()) {
    return (
      <a
        ...
        className="flex items-center gap-3 rounded-lg border border-gray-200 bg-white p-4 hover:bg-gray-50 transition-colors min-h-[44px]"
      >
        <span
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-100 text-red-600 text-sm font-bold"
          aria-hidden
        >
          ▶
        </span>
        <span className="text-sm font-medium text-gray-800">Watch on YouTube — {title}</span>
      </a>
    )
  }
...
export default function CommonIssuesPage() {
  ...
  // Reset expanded issue whenever the user switches category
  ...
  return (
    <PageContainer className="space-y-4 md:space-y-6">
        <header>
          <p className="text-xs font-semibold uppercase tracking-wide text-green-600">Reference</p>
          <h1 className="text-xl md:text-2xl font-bold text-gray-900 mt-0.5">Common Issues</h1>
          <p className="text-sm text-gray-500 mt-1">
            Identify, prevent, and treat common plant problems
          </p>
        </header>

        {/* Categories — each one is its own accordion that expands inline */}
        <div className="space-y-3">
          {COMMON_ISSUES.map((category) => {
            const isSelected = selectedCategory === category.name
            const borderColor = { red: 'border-red-400', orange: 'border-orange-400', yellow: 'border-yellow-400', blue: 'border-blue-400' }[category.color] ?? 'border-gray-300'

            return (
              <div key={category.name} className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">

                {/* Category header — clicking this expands/collapses */}
                <button
                  type="button"
                  onClick={() => handleSelectCategory(category.name)}
                  aria-expanded={isSelected}
                  className={`w-full flex items-center justify-between px-4 py-4 text-left transition-colors min-h-[56px] ${isSelected ? 'bg-gray-50' : 'hover:bg-gray-50'}`}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{category.icon}</span>
                    <div>
                      <h2 className="text-base font-semibold text-gray-800">{category.name}</h2>
                      <p className="text-xs text-gray-500">{category.description} · {category.issues.length} issues</p>
                    </div>
                  </div>
                  <svg className={`w-5 h-5 text-gray-400 transition-transform flex-shrink-0 ${isSelected ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>

                {/* Issues nested directly under this category */}
                {isSelected && (
                  <div className={`border-t-2 ${borderColor} divide-y divide-gray-100`}>
                ...
                >
                  <button
                    ...
                    aria-expanded={expandedIssue === issue.name}
                    className="w-full px-4 py-3 text-left flex items-center justify-between hover:bg-gray-50 transition-colors min-h-[52px]"
                  >
                    <div>
